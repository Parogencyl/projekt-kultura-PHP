<?php
    ...
    if($params['error']){
        if($params['error'] === 'add'){
            $error = "Nie udało się dodać warsztatu.";
        } 
    }

    if($params['success']){
        if($params['success'] === 'added'){
            $success = "Warsztat został dodany bez obrazów.";
        } else if($params['success'] === 'allAdded'){
            $success = "Warsztat oraz jego obrazy zostały dodane.";
        }
    }
?>
...
<section class="container mb-5 mt-5">
    ...
    <form action="/projekt-kultura/admin.php/?page=addWorkshop" method="POST" enctype="multipart/form-data">

        <div class="form-group mb-3">
            <input type="text" class="form-control" name="name" placeholder="Nazwa warsztatu"
                required>
        </div>

        <div class="form-group">
            <textarea class="form-control" name="description" rows="4" placeholder="Główny tekst" required></textarea>
        </div>

        <div class="form-check mb-4">
            <input type="checkbox" class="form-check-input" id="exampleCheck1" name="forSale">
